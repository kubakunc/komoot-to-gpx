# Play Store listing — Nederlands (nl-NL)

Play Console → Main store listing → **Add language → Dutch (Netherlands)**.
Titel blijft Engels (merknaam bevat al "GPX" + "Komoot").

## Short description (≤ 80 tekens)

```
Komoot tochten exporteren als GPX — voor Garmin, Wahoo en elke fietscomputer.
```

## Full description (≤ 4000 tekens)

```
Met Export GPX for Komoot download je je eigen Komoot tochten als standaard GPX-bestanden — ook privétochten — zodat je ze kunt laden op je GPS-horloge, je fietscomputer of elke navigatie-app die GPX importeert.

Je logt in bij Komoot op hun eigen inlogpagina (wij zien je wachtwoord nooit). De app toont daarna elke tocht uit je account, laat van elke route een voorbeeld op een echte kaart zien en bewaart elke tocht met één tik als GPX-bestand op je telefoon. Je kunt een tocht ook rechtstreeks vanuit de Komoot-app delen naar deze app — hij opent hier, klaar om te exporteren.

WAT JE KUNT DOEN
• Blader door elke opgenomen rit en elke geplande route in je account.
• Filter de lijst op alleen Voltooide of alleen Geplande tochten.
• Bekijk een voorbeeld van elke route op een echte kaart voordat je exporteert.
• Deel een tocht vanuit de Komoot-app — hij opent hier, klaar om op te slaan (Delen → Export GPX).
• Sla een tocht op als standaard .gpx-bestand waar je maar wilt op je telefoon — Downloads, Drive, Dropbox.
• Laad het GPX-bestand in Garmin Connect, Wahoo, Hammerhead, Bryton, Coros, Suunto, Apple Watch, OsmAnd, Locus Map, Gaia of elke andere app die GPX importeert.

JOUW ROUTES, ELK APPARAAT
Een route die je hebt gepland of opgenomen is van jou. Door hem als GPX-bestand te exporteren kun je navigeren met elk apparaat dat je wilt — een Garmin- of Wahoo-fietscomputer, een sporthorloge of een andere kaart-app — terwijl je blijft plannen in Komoot.

PRIVACY
De app heeft geen eigen server. Het inloggen bij Komoot gebeurt in een in-app browser op komoot.com — wij zien je e-mailadres of wachtwoord nooit. Tochtgegevens gaan rechtstreeks tussen je telefoon en de servers van Komoot; wij sturen ze niet door en slaan ze niet op.

ADVERTENTIES
De app is gratis en toont een kleine banneradvertentie. In de EER vraagt de app om toestemming voordat er gepersonaliseerde advertenties worden getoond (je kunt weigeren en alles blijven gebruiken).

NIET GELIEERD AAN KOMOOT
Dit is een tool van derden. "Komoot" is een geregistreerd handelsmerk van komoot GmbH. Wij zijn niet gelieerd aan of goedgekeurd door komoot GmbH. De app gebruikt je eigen account om je eigen gegevens op te halen.

ONDERSTEUNING
Een bug gevonden? Lukt het exporteren van een tocht niet? Mail naar contact@velologic-labs.eu — meestal nog dezelfde dag antwoord.

Privacybeleid: https://kubakunc.github.io/komoot-to-gpx/privacy-policy/
```

## Zoektermen (ter oriëntatie, niet invoerbaar)

komoot gpx exporteren, komoot tocht downloaden, komoot garmin, komoot wahoo, gpx bestand, fietscomputer route
