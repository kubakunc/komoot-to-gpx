# Play Store listing — Français (fr-FR)

Play Console → Main store listing → **Add language → French (France)**.
Le titre reste en anglais (le nom de la marque contient déjà « GPX » + « Komoot »).
...
```
Avec Export GPX for Komoot, téléchargez vos propres itinéraires Komoot sous forme de fichiers GPX standard — y compris les privés — pour les charger sur votre montre GPS, votre compteur vélo ou n'importe quelle appli de navigation qui importe le GPX.

Vous vous connectez à Komoot sur leur propre page de connexion (nous ne voyons jamais votre mot de passe). L'appli affiche ensuite chaque itinéraire de votre compte, montre un aperçu de chaque tracé sur une vraie carte et enregistre n'importe quel tour en fichier GPX d'une seule touche. Vous pouvez aussi partager un itinéraire depuis l'appli Komoot directement vers celle-ci — il s'ouvre ici, prêt à exporter.

CE QUE VOUS POUVEZ FAIRE
• Parcourez chaque sortie enregistrée et chaque itinéraire planifié de votre compte.
• Filtrez la liste pour afficher uniquement les tours Terminés ou uniquement les Planifiés.
• Visualisez un aperçu de chaque tracé sur une vraie carte avant d'exporter.
• Partagez un itinéraire depuis l'appli Komoot — il s'ouvre ici, prêt à enregistrer (Partager → Export GPX).
...
Politique de confidentialité : https://kubakunc.github.io/komoot-to-gpx/privacy-policy/
```
